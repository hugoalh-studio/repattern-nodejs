const patternList = require("./pattern.js");
	...
	for (let index = 0; index < patternListKey.length; index++) {
		...
		if (name.search(new RegExp(`^${patternNameMap[key] || key}$`, "giu")) === 0) {
			...
			if (patternNoFlag.includes(key) === false) {
				if (flagSet.includes("b") === true && flagSet.includes("e") === false) {
					resultPattern = `^${resultPattern}(?=\\s+)|(?<=\\s+)${resultPattern}(?=\\s+)|(?<=\\s+)${resultPattern}$`;
				};
				if (flagSet.includes("b") === false && flagSet.includes("e") === true) {
